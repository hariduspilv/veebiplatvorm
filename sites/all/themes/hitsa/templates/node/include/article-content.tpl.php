            ...
            <div class="row">
              <?php if ($node->type === 'content_page' && $node->cp_type[LANGUAGE_NONE][0]['value'] === 'cp_simple_without_images') : ?>
                ...
                <?php else : ?>
                  ...
                  <?php if (
                    !empty($cinfo_homepage_url) || !empty($cinfo_e_mail)
...
<!--                      </figure class="sm-hide">-->
                  ...
                  <?php if (!empty($article_video)) : ?>
                    <figure>
                      <a href="<?php print file_create_url($article_video[0]['uri']); ?>" data-title="<?php print check_plain($article_video[0]['filename']); ?>" data-plugin="modal" data-modal="video-<?php print $article_video[0]['fid']; ?>" data-closebutton="<?php print t('Close'); ?>" data-heading="<?php print check_plain($article_video[0]['filename']); ?>" <?php if (!empty($image['field_file_image_title_text'])) print ' data-title="' . check_plain($article_video[0]['filename']) . '"'; ?>>
                        <?php if (!empty($video_thumbnail)) : ?>
                          <img src="<?php print $video_thumbnail; ?>" alt="<?php print check_plain($article_video[0]['filename']); ?>">
                        <?php else : ?>
                          <img src="<?php print image_style_url('hitsa_core_thumbnail', $image['uri']); ?>" alt="<?php if (!empty($image['field_file_image_alt_text'])) print check_plain($image['field_file_image_alt_text'][LANGUAGE_NONE][0]['value']); ?>">
                        <?php endif; ?>
                      </a>
